...
const ParticleMaterial = shaderMaterial(
    ...
    uniform float uMixShape4;
    uniform float uMixShape5;
    uniform float uMixShape6;
    ...
    void main() {
      // Morphing Logic: Now includes aStart and 6 shapes
...
const fillTextPoints = (buffer, startIndex, text, count, isMobile) => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const wy = -(py - height / 2) * scale - (isMobile ? 2.1 : 2.5);
...
const generateShapePoints = (count, shapeIndex, isMobile) => {
    ...
    if (shapeIndex === 0) { // Shield
    ...
    } else if (shapeIndex === 3) { // User (Candidates)
        textLabel = "CANDIDATES";
        const headCount = Math.floor(SHAPE_COUNT * 0.35);
        const bodyCount = SHAPE_COUNT - headCount;
        ...
        for (let i = 0; i < SHAPE_COUNT; i++) {
            ...
            if (i < headCount) {
                // Head: Circle
                const t = i / headCount;
                writePointOnThickArc(points, i, 0, 0.5, 0, 0.4, 0, Math.PI * 2, r, t, angle);
            } else {
                // Body: Shoulders
                const k = i - headCount;
                const t = k / bodyCount;
                writePointOnThickArc(points, i, 0, -0.8, 0, 0.8, Math.PI * 0.1, Math.PI * 0.9, r, t, angle);
            }
        }
    } else if (shapeIndex === 4) { // Gear (Settings)
        textLabel = "SETTINGS";
        // Simpler approach: inner hole, outer ring, and radial spokes as teeth
        const innerHoleCount = Math.floor(SHAPE_COUNT * 0.15);
        const outerRingCount = Math.floor(SHAPE_COUNT * 0.35);
        const teethCount = SHAPE_COUNT - innerHoleCount - outerRingCount;
        const numTeeth = 8;
        ...
        for (let i = 0; i < SHAPE_COUNT; i++) {
            ...
            if (i < innerHoleCount) {
                ...
                const t = i / innerHoleCount;
                writePointOnThickArc(points, i, 0, 0, 0, 0.25, 0, Math.PI * 2, r, t, angle);
            } else if (i < innerHoleCount + outerRingCount) {
                // Outer circle (gear body)
                const k = i - innerHoleCount;
                const t = k / outerRingCount;
                writePointOnThickArc(points, i, 0, 0, 0, 0.65, 0, Math.PI * 2, r, t, angle);
            } else {
                // Teeth as radial lines extending from outer ring
                const k = i - innerHoleCount - outerRingCount;
                const toothIdx = Math.floor((k / teethCount) * numTeeth);
                const toothAngle = (toothIdx / numTeeth) * Math.PI * 2;
                const withinTooth = ((k / teethCount) * numTeeth) % 1;

                // Each tooth has 3 parts: left edge, top, right edge
                const segment = Math.floor(withinTooth * 3);
                const segmentT = (withinTooth * 3) % 1;
                ...
                if (segment === 0) {
                    // Left edge (going outward)
                    const x1 = Math.cos(toothAngle - toothWidth) * innerR;
                    const y1 = Math.sin(toothAngle - toothWidth) * innerR;
                    const x2 = Math.cos(toothAngle - toothWidth * 0.3) * outerR;
                    const y2 = Math.sin(toothAngle - toothWidth * 0.3) * outerR;
                    writePointOnThickLine(points, i, x1, y1, 0, x2, y2, 0, r, segmentT, angle);
                } else if (segment === 1) {
                    // Top of tooth
                    const x1 = Math.cos(toothAngle - toothWidth * 0.3) * outerR;
                    const y1 = Math.sin(toothAngle - toothWidth * 0.3) * outerR;
                    const x2 = Math.cos(toothAngle + toothWidth * 0.3) * outerR;
                    const y2 = Math.sin(toothAngle + toothWidth * 0.3) * outerR;
                    writePointOnThickLine(points, i, x1, y1, 0, x2, y2, 0, r, segmentT, angle);
                } else {
                    // Right edge (going inward)
                    const x1 = Math.cos(toothAngle + toothWidth * 0.3) * outerR;
                    const y1 = Math.sin(toothAngle + toothWidth * 0.3) * outerR;
                    const x2 = Math.cos(toothAngle + toothWidth) * innerR;
                    const y2 = Math.sin(toothAngle + toothWidth) * innerR;
                    writePointOnThickLine(points, i, x1, y1, 0, x2, y2, 0, r, segmentT, angle);
                }
            }
        }
    } else if (shapeIndex === 5) { // Clipboard with checkmark (Review)
        textLabel = "REVIEW";
        const clipCount = Math.floor(SHAPE_COUNT * 0.15); // Clip at top
        const boardCount = Math.floor(SHAPE_COUNT * 0.45); // Board outline
        const checkCount = SHAPE_COUNT - clipCount - boardCount; // Checkmark inside

        // Board dimensions
        const bw = 0.9, bh = 1.3;
        const bt = 0.6, bb = -1.0; // top and bottom y
        ...
        for (let i = 0; i < SHAPE_COUNT; i++) {
            ...
            if (i < clipCount) {
                ...
                const k = i;
                const side = k % 4;
                const t = (Math.floor(k / 4) / (clipCount / 4));
                const cw = 0.3, ch = 0.2;
                const cy = bt + 0.15;
                if (side === 0) writePointOnThickLine(points, i, -cw, cy + ch, 0, cw, cy + ch, 0, r, t, angle);
                else if (side === 1) writePointOnThickLine(points, i, cw, cy + ch, 0, cw, cy, 0, r, t, angle);
                else if (side === 2) writePointOnThickLine(points, i, cw, cy, 0, -cw, cy, 0, r, t, angle);
                else writePointOnThickLine(points, i, -cw, cy, 0, -cw, cy + ch, 0, r, t, angle);
            } else if (i < clipCount + boardCount) {
                // Board outline (rounded rectangle)
                const k = i - clipCount;
                const side = k % 4;
                const t = (Math.floor(k / 4) / (boardCount / 4));
                if (side === 0) writePointOnThickLine(points, i, -bw, bt, 0, bw, bt, 0, r, t, angle);
                else if (side === 1) writePointOnThickLine(points, i, bw, bt, 0, bw, bb, 0, r, t, angle);
                else if (side === 2) writePointOnThickLine(points, i, bw, bb, 0, -bw, bb, 0, r, t, angle);
                else writePointOnThickLine(points, i, -bw, bb, 0, -bw, bt, 0, r, t, angle);
            } else {
                // Checkmark inside
                const k = i - clipCount - boardCount;
                const shortCount = Math.floor(checkCount * 0.3);
                if (k < shortCount) {
                    const t = k / shortCount;
                    writePointOnThickLine(points, i, -0.5, -0.1, 0, -0.1, -0.5, 0, r, t, angle);
                } else {
                    const t = (k - shortCount) / (checkCount - shortCount);
                    writePointOnThickLine(points, i, -0.1, -0.5, 0, 0.6, 0.3, 0, r, t, angle);
                }
            }
        }
    }
...
const ParticleSystem = ({ themeColor, isPaused, xOffset = 0, stepIndex }) => {
    const COUNT = 4000;
    ...
    useEffect(() => {
        ...
        // If stepIndex is provided, we control manually
        if (typeof stepIndex !== 'undefined') {
            // Map step to shape
            // Step 0 (Details) -> 1 (Checkmark/Voting)
            // Step 1 (Candidates) -> 3 (User/Candidates)
            // Step 2 (Settings) -> 4 (Gear/Settings)
            // Step 3 (Review) -> 5 (Clipboard/Review)
            let target = 1; // Default to Checkmark/Voting
            if (stepIndex === 0) target = 1;
            else if (stepIndex === 1) target = 3;
            else if (stepIndex === 2) target = 4;
            else if (stepIndex === 3) target = 5;

            setTargetShape(target);
            return;
        }

        // Auto-cycle shapes every 6s if no stepIndex
        const interval = setInterval(() => {
            setTargetShape(prev => {
                // Cycle 0 -> 1 -> 2 -> 0 (ignore 3 for hero auto-cycle as per instruction)
                return (prev + 1) % 3;
            });
        }, 6000);
    ...
    useFrame((state, delta) => {
        ...
        materialRef.current.uMixShape4 += (t4 - materialRef.current.uMixShape4) * morphSmoothness;
    ...
    return (
        <points ref={meshRef} position={posCheck} scale={[responsiveScale, responsiveScale, responsiveScale]} frustumCulled={false}>
            <bufferGeometry>
                ...
                <bufferAttribute attach="attributes-aShape4" count={COUNT} array={aShape4} itemSize={3} />
                <bufferAttribute attach="attributes-aShape5" count={COUNT} array={aShape5} itemSize={3} />
                <bufferAttribute attach="attributes-aShape6" count={COUNT} array={aShape6} itemSize={3} />
