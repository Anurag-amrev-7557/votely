...
const dummyActivity = [
  { type: 'login', desc: 'Logged in', time: '2024-06-01 10:23', device: 'Chrome on Mac', ip: '192.168.1.2', location: 'New York, USA', icon: <KeyIcon className="w-5 h-5 text-blue-500" /> },
  { type: '2fa', desc: 'Enabled 2FA', time: '2024-05-30 09:10', device: 'Chrome on Mac', ip: '192.168.1.2', location: 'New York, USA', icon: <ShieldCheckIcon className="w-5 h-5 text-green-500" /> },
  { type: 'password', desc: 'Changed password', time: '2024-05-20 14:45', device: 'Safari on iPhone', ip: '192.168.1.3', location: 'San Francisco, USA', icon: <LockClosedIcon className="w-5 h-5 text-yellow-500" /> },
  { type: 'login', desc: 'Logged in', time: '2024-05-18 08:00', device: 'Firefox on Windows', ip: '192.168.1.4', location: 'London, UK', icon: <KeyIcon className="w-5 h-5 text-blue-500" /> },
  { type: '2fa', desc: 'Disabled 2FA', time: '2024-05-15 12:30', device: 'Edge on Windows', ip: '192.168.1.5', location: 'Berlin, DE', icon: <ShieldCheckIcon className="w-5 h-5 text-red-500" /> },
  { type: 'password', desc: 'Changed password', time: '2024-05-10 14:45', device: 'Safari on iPhone', ip: '192.168.1.3', location: 'San Francisco, USA', icon: <LockClosedIcon className="w-5 h-5 text-yellow-500" /> },
];
const dummySessions = [
...
const initialTips = [
  { id: 1, type: 'warning', icon: <ExclamationTriangleIcon className="w-6 h-6 text-yellow-500 mr-3" />, text: 'Your password is over 90 days old. Consider changing it.', action: 'Change Password' },
  { id: 2, type: 'info', icon: <CheckCircleIcon className="w-6 h-6 text-blue-500 mr-3" />, text: 'No unusual login activity detected.', action: null },
];
    ...
    <div className="max-w-4xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        ...
        {tips.map(tip => (
          <div key={tip.id} className={`flex items-center p-4 rounded-lg border-l-4 ${tip.type === 'warning' ? 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-400' : 'bg-blue-50 dark:bg-blue-900/20 border-blue-400'}`}>
            ...
            <div className="flex-1">
              ...
              {tip.action && <button className="text-xs text-blue-600 dark:text-blue-400 hover:underline mt-1">{tip.action}</button>}
            </div>
            <button onClick={() => setTips(tips.filter(t => t.id !== tip.id))} className="ml-2 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200" aria-label="Dismiss">
              &times;
            </button>
          </div>
        ))}
      ...
      <motion.div layout className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-8 p-6 hover:shadow-md transition-shadow duration-200">
        ...
        {is2FAEnabled && (
          <div className="mt-4 flex flex-col gap-2">
            <button onClick={() => setShowBackupCodes(v => !v)} className="text-xs text-blue-600 dark:text-blue-400 hover:underline font-medium mb-1 w-fit">{showBackupCodes ? 'Hide' : 'Show'} Backup Codes</button>
            <button onClick={() => setShowRegenerateCodes(true)} className="text-xs text-yellow-600 dark:text-yellow-400 hover:underline font-medium mb-1 w-fit">Regenerate Backup Codes</button>
            <AnimatePresence>
              {showBackupCodes && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-gray-50 dark:bg-gray-800 rounded p-4 flex flex-wrap gap-2">
                  ...
                  <button className="ml-2 text-xs text-blue-600 dark:text-blue-400 hover:underline">Copy All</button>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
      ...
      <motion.div layout className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-8 p-6 hover:shadow-md transition-shadow duration-200">
              ...
              {dummySessions.map(session => (
                <tr key={session.id} className="border-b border-gray-100 dark:border-gray-800">
                  ...
                  <td className="py-2 px-2 text-right">
                    {!session.current && (
                      <button onClick={() => { setSelectedSession(session); setShowRevokeModal(true); }} className="text-xs text-red-600 hover:underline">Revoke</button>
                    )}
                  </td>
                </tr>
              ))}
        ...
        <div className="flex justify-end mt-3">
          <button className="px-3 py-1 text-xs font-semibold text-red-600 hover:text-red-700 rounded transition-all duration-200 flex items-center gap-1"><ArrowRightOnRectangleIcon className="w-4 h-4" />Revoke all sessions except this</button>
        </div>
      </motion.div>
      ...
      <motion.div layout className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow duration-200">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:gap-4 mb-4 gap-2">
          ...
          />
          <select value={activityType} onChange={e => { setActivityPage(1); setActivityType(e.target.value); }} className="px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white text-sm">
        ...
        {totalPages > 1 && (
          <div className="flex justify-end items-center gap-2 mt-4">
            <button disabled={activityPage === 1} onClick={() => setActivityPage(p => Math.max(1, p-1))} className="px-2 py-1 text-xs rounded bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 disabled:opacity-50">Prev</button>
            <span className="text-xs text-gray-500 dark:text-gray-400">Page {activityPage} of {totalPages}</span>
            <button disabled={activityPage === totalPages} onClick={() => setActivityPage(p => Math.min(totalPages, p+1))} className="px-2 py-1 text-xs rounded bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 disabled:opacity-50">Next</button>
          </div>
        )}
      </motion.div>
      ...
      <AnimatePresence>
        {showPwdModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white dark:bg-gray-900 rounded-xl shadow-xl p-8 w-full max-w-md">
              <h3 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">Change Password</h3>
                ...
                <ul className="mt-2 space-y-1">
                  {pwdReqs.map((req, i) => (
                    <li key={i} className={`flex items-center gap-2 text-xs ${req.met ? 'text-green-600 dark:text-green-400' : 'text-gray-400 dark:text-gray-500'}`}>
                      <CheckCircleIcon className={`w-4 h-4 ${req.met ? 'text-green-500' : 'text-gray-300 dark:text-gray-600'}`} />
                      {req.label}
                    </li>
                  ))}
                </ul>
      ...
      <AnimatePresence>
        {show2FAModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white dark:bg-gray-900 rounded-xl shadow-xl p-8 w-full max-w-md">
              <h3 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">Enable 2FA</h3>
              <div className="mb-4">
                ...
                <div className="mb-3 flex items-center gap-2">
                  ...
                  <button className="text-xs text-blue-600 dark:text-blue-400 hover:underline">Copy</button>
                </div>
      ...
      <AnimatePresence>
        {showRegenerateCodes && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white dark:bg-gray-900 rounded-xl shadow-xl p-8 w-full max-w-md">
              <h3 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">Regenerate Backup Codes</h3>
      ...
      <AnimatePresence>
        {showDisable2FAConfirm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white dark:bg-gray-900 rounded-xl shadow-xl p-8 w-full max-w-md">
              <h3 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">Disable 2FA</h3>
      ...
      <AnimatePresence>
        {showRevokeModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white dark:bg-gray-900 rounded-xl shadow-xl p-8 w-full max-w-md">
              <h3 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">Revoke Session</h3>
