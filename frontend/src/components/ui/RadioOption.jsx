...
const RadioOption = React.forwardRef(({ option, isSelected, onSelect, index }, ref) => (
  <div
    ...
    className={`group relative flex items-start p-4 sm:p-6 rounded-xl border cursor-pointer transition-all duration-300 w-full ${isSelected
      ...
      <div className="flex items-start gap-4 sm:gap-6 w-full">
        {option.image && (
          <div className="relative flex-shrink-0">
            <div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-full overflow-hidden transition-all duration-300 ${isSelected
        ...
        <div className="flex-1 min-w-0 w-full">
          <div className="flex items-center justify-between gap-3 sm:gap-4 w-full">
            <h3 className={`text-base sm:text-lg font-semibold ${isSelected
                ? 'text-blue-900 dark:text-[#c7d7e9]'
                : 'text-gray-900 dark:text-white'
              }`}>
              {option.text}
            </h3>
            <div className={`h-5 w-5 sm:h-6 sm:w-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${isSelected
          ...
          {option.description && (
            <p className={`mt-1 text-xs sm:text-sm ${isSelected
          ...
          {option.party && (
            <div className={`mt-2 sm:mt-3 inline-flex items-center px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full text-xs font-medium ${isSelected
